...
%%\noindent{\centering\includegraphics[width=0.95\linewidth]{images/cluded.pdf}\\}
  ...
  The OpenCV function FindContours give us a list of potential contours of object we have to analyse in order to filter the shapes we want to draw in our scene.
  
  For that we use the OpenCV ApproxPoly function which return a polygon we have to check whether it's a triangle, a quadrilateral, etc.
  
  After processing all the shapes in the frame, we just create a room, in our game level, associated with the shapes retrieved with OpenCV.
